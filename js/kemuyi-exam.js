/**
 * 小轿车驾驶科目一考试页面脚本
 * 功能：类别选择、加载题库、随机抽题、答题交互、答案判断、解析显示、成绩统计
 */
...
class KemuYiExamApp {
  constructor() {
    // 配置
    this.config = {
      indexUrl: '/data/kemuyi-questions/index.json',  // 科目一题库索引文件
      questionsBaseUrl: '/data/kemuyi-questions/',     // 科目一题库文件基础路径
      questionsPerExam: 100,  // 每次考试抽取100题（科目一标准）
      passScore: 90  // 及格分数90分
    };
  ...
  renderCategoryCards() {
    ...
    const allCard = this.createCategoryCard({
      id: 'all',
      name: '全部类别',
      description: '从所有6个类别的300道题中随机抽取100题',
      file: '',
      question_count: 300
    }, true);
  ...
  showResults() {
    ...
    // 根据分数设置图标和标题（科目一及格标准：90分）
    let iconClass, icon, title, subtitle;
    
    if (score >= this.config.passScore) {
      iconClass = 'excellent';
      icon = '🎉';
      title = '恭喜通过！';
      subtitle = `你已达到科目一及格标准（${this.config.passScore}分），继续保持！`;
    } else if (score >= 80) {
      iconClass = 'good';
      icon = '💪';
      title = '接近及格！';
      subtitle = `还差一点就及格了（需${this.config.passScore}分），再接再厉！`;
    } else {
      iconClass = 'poor';
      icon = '📚';
      title = '需要努力！';
      subtitle = `建议复习相关知识点，多做练习，争取达到${this.config.passScore}分！`;
    }
